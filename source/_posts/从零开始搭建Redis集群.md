...
![redis](D:/git/CKING.github.io/source/_posts/从零开始搭建Redis集群/redis.png)
...
```
 ...
 ![开启启动](D:/git/CKING.github.io/source/_posts/从零开始搭建Redis集群/开启启动.png)
...
3. 执行` redis-cli --cluster check 192.168.0.114:7007`查看redis cluster的情况，可以看到7007实例已经作为master新增到redis cluster中。但这个master只有0个hash slots，所以我们还要给他分配hash slots.
   ...
   ![add-node](D:/git/CKING.github.io/source/_posts/从零开始搭建Redis集群/add-node.png)
...
![4096](D:/git/CKING.github.io/source/_posts/从零开始搭建Redis集群/4096.png)
...
![slave](D:/git/CKING.github.io/source/_posts/从零开始搭建Redis集群/slave.png)
...
2. 假设我们要删除7007节点，先执行`redis-cli --cluster reshard 192.168.0.112:7001`，然后输入1365，将1365个slot迁移到其中一个master中。然后再依次迁移1365和1366个slot到另外两个master中。此时7007零个slots。
   ...
   ![del](D:/git/CKING.github.io/source/_posts/从零开始搭建Redis集群/del.png)
